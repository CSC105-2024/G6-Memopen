...
export const createPost = async (c:Context)=>{
    try{
        ...
        const newPost = await postModel.addPost(userId, json, tag,thumbnail);
        return c.json({
            success:true,
            data:newPost,
...
export const editPost = async (c:Context)=>{
    try{
        ...
        const body = await c.req.json();
        const { json, thumbnail } = body;
        const updatedPost = await postModel.editPost(postId, json, thumbnail);
        return c.json({
            sucess:true,
            data:updatedPost,
